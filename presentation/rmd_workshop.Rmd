...
- Unzip and rename the folder
  ...
  - rename the folder as `YOURNAME-rmd`
     - e.g., `resul-rmd`
     - this will come handy when we collaborate [Part 9](#part9)
...
<sup>*</sup> The text, _Lorem ipsum_, is generated with the `stringi` package `r Citep(bibliography, "R-stringi")` while the dataset is created with the `fabricatr` package `r Citep(bibliography, "R-fabricatr")`. 
...
`data\journals.csv`
- a dataset created with the `fabricatr` package `r Citep(bibliography, "R-fabricatr")`, imagined to explore the `Google Scholar` rankings of fictitious journals
...
![](rmd_workshop_files/images_data/window_global_options.png)
...
![](rmd_workshop_files/images_data/window_project_options.png)
...
- `rmarkdown` `r Citep(bibliography, "R-rmarkdown")`, for automating the process of converting R Markdown documents into other formats
...
- `tinytex` `r Citep(bibliography, "R-tinytex")`, for PDF outputs
...
- `dplyr` `r gsub("Ã§", "ç", Citep(bibliography, "R-dplyr"))`, for data manipulation
    - popular alternative: e.g., `base` `r Citep(bibliography, "R-base")`, `data.table` `r Citep(bibliography, "R-data.table")`
...
- `stargazer` `r Citep(bibliography, "R-stargazer")`, for tables
    - popular alternatives: `knitr` `r Citep(bibliography, "R-knitr")`, `kableExtra` `r Citep(bibliography, "R-kableExtra")`, `huxtable` `r Citep(bibliography, "R-huxtable")`
...
- `ggplot2` `r Citep(bibliography, "ggplot2")`, for figures
    - popular alternatives: `base` `r Citep(bibliography, "R-base")`, `plotly` `r Citep(bibliography, "R-plotly")`
...
- R Markdown: The Definitive Guide `r Citep(bibliography, "rmarkdown2018")`
...
<sup>*</sup> During the workshop, [R Markdown Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf) is likely to be more helpful than these resources, which I recommend to be consulted after the workshop.
...
![](rmd_workshop_files/images_data/toolbar.png)

<br>

These include, most impotantly,

- the ![](rmd_workshop_files/images_data/button_knit.png) button to compile .Rmd documents
...
- R Markdown can produce more than documents,<sup>*</sup> including
  ...
  - books, with `bookdown` `r Citep(bibliography, "R-bookdown")`
  - websites, with `blogdown` `r Citep(bibliography, "R-blogdown")`
...
<br>
    - the list of available variables, as well as options and sub-options for these variables, depends on the output format
       ...
       - [R Markdown Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf) provides a helpful list
...
<sup>*</sup> For reasons of simplicity, this workshop focuses on LaTex and/or PDF outputs. Different output formats have slightly different customisations. See [Pandoc User's Guide](https://pandoc.org/MANUAL.html) and/or [R Markdown Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf).
...
-  see, for example, the [R Markdown Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf)
...
- R Markdown follows the syntax in Pandoc's Markdown 
   ...
   - for a useful summary of the syntax, see the [R Markdown Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf)
...
- The complete list of options is available at <https://yihui.org/knitr/options>
  - [R Markdown Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf) provides a helpful list as well
...
Use the `doublespacing` command from the LaTeX package `setspace` `r Citep(bibliography, "setspace")`
- because the command comes from a package, we need to add it to YAML with `header-includes`
- including commands in YAML ensures they are applied through the output<sup>*</sup>  
...
2. .yellow-h[project setup], repeated for every RStudio project
...
- enter the following lines in the `Terminal`, with .yellow-h[your username and repository name]
